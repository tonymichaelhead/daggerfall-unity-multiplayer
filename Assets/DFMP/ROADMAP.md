...
- The current deterministic marker scan plus curated enemy-type list is a temporary development/testing scaffold only. It must not be presented as M8-complete behavior and must be removed or isolated behind an explicit development-only mode before the M8 closeout gate.
...
- `DungeonRosterSize` is a temporary scaffold setting and must not override native roster count in the MVP parity mode. Replace it with an explicit development-only override or a parity-safe configuration model after native descriptor generation is implemented.
