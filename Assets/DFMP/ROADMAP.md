...
- Native enemy AI parity: see the dedicated subsection below. Item 1 (native give-up timer and full classic detection) is delivered; items 2–6 remain.
...
Progress: item 1 delivered; items 2–6 open.

M8 shipped a deliberately simplified server AI: beeline pursuit and attack-on-cooldown inside a flat range. M9 replaces it so enemies read as native Daggerfall enemies to any player who has played single-player DFU. Detection and give-up now match native DFU; pursuit destination, combat movement patterns, attack cadence, and obstacle/ledge handling are still outstanding.
